...
| `docs/specs/Test_Automation_Strategy.md` | Test tiers and tools, the invariant-to-test map with its execution-based gate, determinism seams (clocks, keys, crash points, simulator ownership), scenario catalogue, privacy assertions, CI jobs and merge gates |
...
- **Testing:** pytest for unit tests, Testcontainers for integration, Playwright (driven from pytest) for end-to-end, Vitest for `web/`, k6 for load. Pyramid 60/30/10 as a guide, 80 % line coverage floor on kernel and modules, green `main`. Crash tests kill a real process; safety tests gate on their first result. (`docs/specs/Test_Automation_Strategy.md`)
