...
> **Depends on:** FTS v1.3 §2 (gates G1–G5), §4 (two-phase commit; §4.6 who runs which step), §5 (idempotency key lifecycle), §6–§7 (Healer, zombie classification), §9 (resource lock), §10 (FIN codes), §11 (escalation) · MR v1.2 §3 (tiers, call graph), §4 (manifest schema), §6 (envelope and `phase`; §6.4 ledger states; §6.6 settings; §6.7 `reconcile`), §7 (isolation), §9 (MOD codes), §10 (worked PAY_BILL) · CM v1.4 §3.2 (AA_BALANCE_FETCH), §4.2 (grant flow), §5 (CONSENT_REVERIFY), §6.1 (AA adapter) · DM v1.4 §3.4, §3.7–§3.9, §6.1, §8 (Sharma seed), §9.3 (module and family-level table conventions) plus the DM v1.3 decisions (lowercase Core-PRD roles, separate `resource_lock` table, kernel schema `core`) · RB v1.2 §2–§4, §8, §9 · FSM v2.1 §2 (tiers), §3.1 (TTL) · Core PRD v2.2 §2, §5, §6 · Tech_Spec_Simulator_Architecture v0.1 §4, §5.1–5.2, §6.1–6.2 (BBPS and AA simulators) · PROJECT_TRACKER Decision Log (2026-09-17) and Phase 1 Build Gate.
...
| v0.3 | 2026-09-21 | Codex review of PR #25 (findings 1, 2, 7) and WP-11 alignment. (f1) The daily family limit is enforced by an atomic per-family, per-IST-day reservation made in the same transaction as the payment row, so two payments to different billers cannot both take the last rupees, and an unconfirmed payment keeps counting (§4.5, §4.6). (f2) A payment is checked at approval against a recorded settings version and re-checked at execution against whatever is **stricter** then; settings writes are version-checked and loosening needs a passkey (§4.6). (f7) `reconcile()` finalises the transaction row, the ledger row and the reservation together, and the same-key resubmission path is spelled out (§8). Also: the **kernel** writes `BILL_PAYMENT_INITIATED` (FTS v1.3 §4.6; the module writes no payment audit row at all); `phase: prepare \| execute`; purpose-bound consent; FIN codes as `domain_code`; OI-3, OI-8, OI-9 closed. | Shantanu Chaudhary (with Claude Code; review by Codex) |
...
2. PERMISSION_CHECK: member ∈ `allowed_roles`; finance active for the family; tier 1 ≤ ceiling 1; an active consent record of Priya for the exact purpose `AA_BALANCE_FETCH` exists (MR v1.2 §7.3 step 6; a handle for another AA purpose would not do). G2: `INSERT INTO resource_lock (family_id, 'BBPS_BESCOM_KA_001', session_id, priya, NOW())` succeeds (FTS §9.2).
3. REASONING (dispatch with `phase: prepare`, MR v1.2 §6.2: no ledger row, no payment call): FinanceAgent calls the DPI Gateway → BBPS simulator bill fetch (`POST /bill/fetch`, SIM §5.1 B1, §6.1) → `{due_paise: 284700, bill_ref: 'KA-2026-09-8812', due_date: 2026-09-25}` with `cache_metadata {fetched_at, source_api: 'bbps_sim', expires_at: +24h}`. G3: a forced AA balance fetch (TTL bypassed, coalesced per RB §3.2, one of three hourly slots for handle `ch-abc-123`) returns 1825040 paise. Check: 1825040 ≥ 284700 + 5000 buffer — pass. The module records what it checked in `finance.prepared_payments` (amount, balance, buffer, the family's `settings_version`, remaining daily limit) and returns it as `result.data.prepared`; `success` here means "ready for approval".
...
6. Phase 1 COMMIT (kernel, one transaction) moves the session to EXECUTION with the key and writes the audit row `BILL_PAYMENT_INITIATED` (FTS v1.3 §4.6): the point of no return. Dispatch with `phase: execute`: FinanceAgent, in one transaction ("A"), re-checks the prepared payment against the settings in force **now** (§4.6), reserves the amount against the day's family limit, and inserts `finance.idempotency_ledger (key, pending)` and `finance.transactions (state = 'INITIATED')`; commits; then asks `payment_routing` to format the BBPS payload and posts it through the DPI Gateway with the key (10 s timeout).
7. BBPS simulator returns 200 with a `transaction_ref_id`, written here as `'BBPS-TXN-2847001'` after FTS §2.3; the simulator derives the reference from the idempotency key (SIM §4.2), so tests assert the pattern and that payment and status agree, not a fixed value. In one transaction ("B") the module commits `transactions → CONFIRMED` and the ledger row `final` with the envelope, then returns `status: success`, `side_effects: [{provider: bbps, external_ref: 'BBPS-TXN-2847001', state: executed_confirmed, amount_paise: 284700}]`.
8. Phase 2 (kernel, one transaction): audit row `BILL_PAYMENT_EXECUTED` (details: biller_id, amount_paise, transaction_ref_id, idempotency_key, automation_tier 1 — identifiers and amounts only) plus session → SUCCESS_CONFIRMATION. Only after COMMIT is the lock released: `UPDATE core.resource_lock SET released_at = NOW(), release_reason = 'completed'` (DM v1.4 §3.11; lock rows are never deleted, and where FTS v1.3 still shows a DELETE its §9.2 note says to run this UPDATE). Then `notification_engine` pushes `finance.pay_bill.success` to Priya Phone in Hindi and to Ravi Phone as the admin default notification. The Kitchen Tablet shows nothing.
...
2. FinanceAgent cannot claim success or failure. It returns `status: failure`, `error: {code: MOD_EXECUTION_UNCONFIRMED, class: TERMINAL, message_key: finance.error.FIN_001}` and `side_effects: [{state: executed_unconfirmed, external_ref: null, amount_paise: 284700}]` (MR §9, §10 step 8b). The ledger keeps `pending`, and the amount stays reserved against today's family limit: money that may have moved counts.
3. Priya sees FIN_001: "भुगतान में सामान्य से अधिक समय लग रहा है। हम स्थिति जांच रहे हैं।" (FTS §10.2) with no retry button. The session stays in EXECUTION and the lock stays held (FTS §4.3; MR v1.2 §6.5 and §9 say the same, also for a crash or a garbled response, and nothing but the Healer may move an EXECUTION session, AGENTS.md §4 item 19).
...
5. T+10 min: next run processes P1 first — re-polls BBPS, writes the audit row with the original BBPS timestamp, moves the session to SUCCESS_CONFIRMATION, marks the task `succeeded` in one transaction, releases the lock and pushes "✅ ₹2,847 paid. Ref: BBPS-TXN-2847002" to Priya and Ravi. Before that Phase 2 transaction the Healer calls `finance.reconcile(key, outcome: executed, external_ref, occurred_at)`, which moves the `finance.transactions` row to CONFIRMED **and** the ledger row to `final` in one transaction (§8); a later re-dispatch with the key therefore finds a final envelope.
6. Variants exercised with other scenario billers (SIM §5.1 B6, B7, B11, B16): `NOT_FOUND` → the Healer re-queues `BILL_PAYMENT` (P3) with the **same** key (FTS §6.4 hard rule), executed as a `phase: execute` dispatch carrying `recovery: {mode: resubmit}`, which is what allows the module to call BBPS for a ledger row that is still `pending` (MR v1.2 §6.4); `PENDING` → wait, admin alert at 30 min, FAILED at 4 h (FTS §7.2); BBPS unreachable three polls → admin alert FIN_011. Crash Scenarios A–D of the Build Gate map onto steps 1–5.
...
- **Information gathering:** actor role from `v_family_members`; consent references from the envelope's `consent[]`, resolved by the kernel for the exact purpose and subject (`v_active_consents` is a UI hint, never the authorisation, DM v1.4 §3.17); device context from `v_device_surfaces` and the envelope's `context.is_public_surface`; the family's linked billers and cached dues from `finance.billers`; BBPS bill fetch through the DPI Gateway (`phase: prepare`, MR v1.2 §10 step 4); the forced AA balance fetch at G3 (TTL bypass, coalescing RB §3.2, 3/hour per handle); the BBPS daily budget from Redis (50/day per user, fail-closed on Redis loss, RB §2.4).
- **Analysis logic (deterministic; the LLM never computes money):** `amount = entities.amount_paise` if given and equal to the fetched due, else the fetched due; a mismatch returns `needs_clarification` with both figures. Gates in FTS order: RBAC → lock → balance (`balance_paise ≥ amount_paise + buffer`, buffer 5000 paise unless the family has raised it, §4.6; else FIN_005; no fresh fetch this session, else FIN_006 hard stop) → passkey (always, `biometric_required_above_paise: 0`) → CONSENT_REVERIFY. The family's daily limit (§4.6) is previewed in `prepare` and enforced atomically in `execute`. FTS §5.3 collision check: a non-terminal session for the same biller in the last 24 h returns that session's status and creates no new key. `deadline_at` is checked before every DPI call (MR §6.2). The automation tier of `PAY_BILL` is always 1.
- **Execution / fulfilment:** the FTS §2.3 sequence. Phase 1 COMMIT (kernel: session → EXECUTION, audit `BILL_PAYMENT_INITIATED`) → `execute` dispatch → module transaction A {re-check against current settings, reserve against the daily limit, ledger `pending`, `transactions INITIATED`} → `payment_routing` formats the payload → DPI Gateway POST with the idempotency key (10 s timeout; BBPS dedups the key for 24 h, so FIN_004 is a success path) → module transaction B {`transactions CONFIRMED` or `FAILED`, ledger `final`, reservation released if FAILED} → response with exhaustive `side_effects` → Phase 2 (kernel, one transaction: audit row + session) → lock release → `notification_engine` push to payer and admin, private devices only, via `display_key`. A timeout or crash yields `MOD_EXECUTION_UNCONFIRMED` with `executed_unconfirmed`, and the Healer owns the outcome. The module never re-prompts, never retries a mutating call on its own, and never generates a second key.
...
Valid against MR v1.2 §4.1 and §4.3. Consent is by exact purpose and subject: `PAY_BILL` and `CHECK_BALANCE` need the actor's `AA_BALANCE_FETCH` (`requires_consent_purposes`); `requires_consent_providers` is the provider set of those purposes (consent providers only: aa, abha, digilocker, ondc); BBPS is a routing provider in `dpi_providers`. Semantic rule S3 forces `biometric_required_above_paise: 0` here. Not shown: `SET_FINANCE_SETTING` (admin only, mutating, entities `{setting, value, expected_settings_version}`; automation tier 0 when the change is stricter, tier 1 with passkey when it is looser, decided by the module in `prepare`).
...
```json
{
  ...
  "scheduled_jobs": [
    { "job_code": "refresh_bill_dues", "handler": "refresh_bill_dues", "every_seconds": 86400, "scope": "family", "max_run_ms": 20000 }
  ],
  "domain_errors": [
    { "domain_code": "FIN_003", "message_key": "finance.error.FIN_003" },
    { "domain_code": "FIN_005", "message_key": "finance.error.FIN_005" },
    { "domain_code": "FIN_013", "message_key": "finance.error.FIN_013" },
    { "domain_code": "FIN_016", "message_key": "finance.error.FIN_016" }
  ],
...
| `finance.family_settings` | The family's adjustable defaults (§4.6) | Family-level table (DM v1.4 §9.3): one row per family; absent row = shipped defaults; `settings_version` |
| `finance.daily_totals` | Money reserved against the family's daily limit, per IST day | The atomic check-and-reserve row (§4.6); family-level |
| `finance.prepared_payments` | What `prepare` checked for a session, for the approval screen and the re-check at `execute` | One row per session; deleted with the transaction's terminal state + 7 days |
| `finance.idempotency_ledger` | MR v1.2 §6.4 ledger: `pending` while the outcome is unknown, `final` with the envelope once known | Rows never garbage-collected before 7 days, and never while `pending` |
...
CREATE TABLE finance.transactions (
  ...
  limit_date               DATE        NOT NULL,               -- IST calendar day this amount is reserved against (§4.6)
  limit_released           BOOLEAN     NOT NULL DEFAULT FALSE,  -- TRUE once a definite failure gave the amount back
  settings_version_applied INTEGER     NOT NULL,               -- family_settings.settings_version checked in transaction A (0 = no row)
...
CREATE TABLE finance.daily_totals (                             -- v0.3: the daily-limit reservation row (§4.6)
  family_id       UUID   NOT NULL REFERENCES core.families(family_id) ON DELETE CASCADE,
  limit_date      DATE   NOT NULL,                              -- IST day: (NOW() AT TIME ZONE 'Asia/Kolkata')::date
  reserved_paise  BIGINT NOT NULL DEFAULT 0 CHECK (reserved_paise >= 0),  -- INITIATED + CONFIRMED amounts of that day
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  PRIMARY KEY (family_id, limit_date)
);

CREATE TABLE finance.prepared_payments (                        -- v0.3: what `prepare` checked, per session
  session_id         UUID        PRIMARY KEY,                   -- supervisor_sessions.session_id, no FK
  family_id          UUID        NOT NULL REFERENCES core.families(family_id) ON DELETE CASCADE,
  user_id            UUID        NOT NULL REFERENCES core.users(user_id) ON DELETE CASCADE,   -- payer
  biller_id          VARCHAR(40) NOT NULL,
  bill_ref           VARCHAR(60),
  amount_paise       BIGINT      NOT NULL CHECK (amount_paise >= 100),
  balance_paise      BIGINT      NOT NULL,                      -- from the forced G3 fetch; 15-minute TTL data, deleted with the row
  balance_checked_at TIMESTAMPTZ NOT NULL,
  buffer_paise       BIGINT      NOT NULL,                      -- buffer in force at prepare
  settings_version   INTEGER     NOT NULL,                      -- 0 = no settings row
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE finance.idempotency_ledger (                       -- MR §6.4
  ...
  response_envelope  JSONB,                                    -- NULL while pending; the ModuleResponse once final, set exactly once
  external_ref       VARCHAR(100),                             -- BBPS reference as soon as one is known
  created_at         TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  finalised_at       TIMESTAMPTZ,
  CHECK ((state = 'final') = (response_envelope IS NOT NULL))
);  -- never collected before 7 days, never while 'pending'
...
Changes go through `SET_FINANCE_SETTING` and are written to the kernel audit log as `FINANCE_SETTINGS_CHANGED` (typed `setting`, `from`, `to`, `direction`, `passkey_verified`; DM v1.4 §6.1) because they change what money may move.

**How a settings change is made safe (v0.3).** One transaction: `UPDATE finance.family_settings SET <col> = $new, settings_version = settings_version + 1 WHERE family_id = $1 AND settings_version = $expected` (first change: INSERT, which conflicts if someone else got there first), plus the audit row. Zero rows updated → FIN_016 "that setting changed in the meantime", and the admin sees the current value. The module classifies the change in `prepare` by comparing `$new` with the **current** value, not with what the client believed: raising or removing the daily limit and lowering the buffer towards ₹50 are *looser* and run at tier 1 (passkey); everything else is *stricter* or a preference and runs at tier 0. Because the write is conditional on the version the classification was made against, two interleaved changes cannot turn a "stricter" approval into a looser result.

**Which settings apply to a payment that is waiting for approval (v0.3).** The stricter of the two moments, always:

1. `prepare` records the amount, the fetched balance, the buffer and `settings_version` V₀ in `finance.prepared_payments`; the approval screen shows those figures.
2. `execute`, transaction A, reads `family_settings` again (version V₁) and re-checks **with the prepared balance** (no second AA fetch: the balance is minutes old and G3's forced fetch already spent one of three hourly slots): `balance_paise ≥ amount_paise + max(buffer at V₀, buffer at V₁)`, and the daily limit at V₁ (below). If V₁ is looser than V₀, V₀'s values still bind this payment: a loosening made while a payment waits never benefits that payment. If the re-check fails the module returns `MOD_DOMAIN_REJECTED` with FIN_005 or FIN_013 and `side_effects: [{state: not_executed}]`; nothing was sent to BBPS, the ledger row is `final` with that refusal, and the kernel's Phase 2 writes `BILL_PAYMENT_FAILED`.
3. `transactions.settings_version_applied` = V₁, so every payment says which rules it was checked under.
4. Not re-checked, because they are not settings: the ₹50 floor, the forced fetch, the passkey for this payment, CONSENT_REVERIFY.

**The daily family limit is a reservation, not a sum (v0.3).** Different billers pay in parallel (§6 row 1), so "add up today's CONFIRMED rows, compare, then pay" lets two payments each see room for themselves. Instead, inside transaction A and before the ledger insert:

```sql
-- $limit = family_settings.daily_limit_paise (skip the whole step when NULL: no family limit)
INSERT INTO finance.daily_totals (family_id, limit_date, reserved_paise)
VALUES ($family_id, (NOW() AT TIME ZONE 'Asia/Kolkata')::date, $amount)
ON CONFLICT (family_id, limit_date) DO UPDATE
  SET reserved_paise = finance.daily_totals.reserved_paise + EXCLUDED.reserved_paise, updated_at = NOW()
  WHERE finance.daily_totals.reserved_paise + EXCLUDED.reserved_paise <= $limit
RETURNING reserved_paise;
-- no row returned -> the limit would be exceeded -> refuse FIN_013, not_executed (and for a first insert
-- the module checks $amount <= $limit before running the statement)
```

- The row lock taken by the upsert serialises the family's payments for the length of transaction A only (no external call happens inside it).
- **What counts:** every payment whose transaction A committed that IST day: INITIATED (outcome unknown, including a timed-out payment the Healer has not resolved yet) and CONFIRMED. **What is given back:** a payment that reaches FAILED with a definite "no money moved" (transaction B, or `reconcile(outcome: failed | not_executed)`), by decrementing the row of its own `limit_date` and setting `limit_released`, once.
- **Day boundary:** 00:00 IST. A payment counts against the day its transaction A ran, even if BBPS confirms it after midnight.
- **Same key, no second reservation:** the reservation and the `transactions` row (unique `idempotency_key`) are inserted in the same transaction; a retry or a Healer resubmission finds the row and skips transaction A altogether (MR v1.2 §6.4).
- The limit read in `prepare` is only a preview for the approval screen ("₹1,200 of today's ₹5,000 left"); it grants nothing.
- Tests: two payers, two billers, last ₹3,000 of the limit, amounts ₹2,000 each, released together → exactly one passes; a timed-out ₹2,847 followed by a ₹3,000 attempt under a ₹5,000 limit → refused until the Healer resolves the first; limit lowered between approval and execution → refused; limit raised between approval and execution → the old limit still binds that payment; a resubmission does not reserve twice.
- This adds no approval step for anyone; it is a refusal, like FIN_005.

**Never a setting** (MR v1.2 §6.6: a setting may make a gate stricter, never weaker): the five gates and their order; a passkey for every BBPS payment whatever the amount; the forced balance fetch before paying, and the hard block when it cannot be made (no override, OI-7); one idempotency key per session and the Healer reusing it; the family-scoped lock; no "try again" control on an unknown outcome; Level 3 automation; the role blocks for minor, elder, staff, managed and passive; the public-surface block; safety alerts (FIN_011, FIN_012, `ROLE_VIOLATION`) reaching the admins.
...
|---|---|---|
| 1 | Two family members pay the same biller concurrently | Family-scoped lock `BBPS_{biller_id}` (FTS §9.2); second session gets FIN_009, no key, nothing external. Different billers proceed in parallel; the family's daily limit is still safe because it is reserved atomically (§4.6). |
...
| 10 | Restricted role (staff / minor) reaches for wealth data | Refused by the kernel before dispatch (`MOD_PERMISSION_DENIED`), audit `ROLE_VIOLATION`, high-severity admin alert (Core PRD §6). |
...
|---|---|---|
| Gate order, 2PC and who runs which step, key lifecycle, Healer, locks, FIN codes | FTS v1.3 §2, §4 (§4.6), §5, §6–§7, §9, §10 | Implements the module side of §2.3; supplies `side_effects`; never approves, never re-keys |
| Manifest, envelope and `phase`, ledger states, settings, `reconcile`, isolation, MOD codes | MR v1.2 §4, §6, §7, §9 | §4.4 manifest; `finance.idempotency_ledger`; imports only `familylifeos.sdk` |
| AA consent, CONSENT_REVERIFY, AA adapter | CM v1.4 §3.2, §4.2, §5, §6.1 | Declares `aa`; consumes `consent[].reverified_at`; rejects > 60 s as `MOD_CONSENT_STALE` |
...
| Table conventions, roles, audit payloads, seed | DM v1.4 §9.3, §6.1, §8 | Schema §4.5; roles `admin`/`member`; `resource_lock` table |
| TTLs and tiers | FSM v2.1 §2, §3.1 | Balance 15 min (forced on pay), bill dues 24 h; tier 1 only |

**Error handling.** Raw codes never reach users (FTS §10.1). `error.code` is always a registered `MOD_*` code, stated exactly in the table; the FIN code travels as `error.domain_code` where the module raises it (registered in the manifest's `domain_errors`) and selects `error.message_key` (`finance.error.FIN_xxx`) (MR v1.2 §9). Mapping of the codes this module raises or relays:
...
| FIN_003 biller not found | `MOD_DOMAIN_REJECTED` / TERMINAL, `domain_code` FIN_003 | No | This bill couldn't be found. Please check your account number. |
| FIN_004 duplicate key ACK | success path | — | Payment already processed. |
| FIN_005 insufficient balance | `prepare`: `MOD_DOMAIN_REJECTED`, `domain_code` FIN_005 (gate G3 fails, the Supervisor releases the lock). `execute` re-check: the same, with `not_executed` | No | Your bank balance is too low for this payment. |
...
| FIN_013 family daily limit | `MOD_DOMAIN_REJECTED` / TERMINAL, `domain_code` FIN_013, `not_executed` | Next IST day, or after an admin raises the limit | Today's family payment limit would be exceeded. |
| FIN_016 settings write refused (bounds or stale version) | `MOD_DOMAIN_REJECTED` / TERMINAL, `domain_code` FIN_016 (module-local code; FTS §10 ends at FIN_015 and is not extended) | After re-reading | That setting changed in the meantime. Here is the current value. |
| FIN_010–FIN_012, FIN_014–FIN_015 | raised by kernel / Healer | per FTS §10.1 | per FTS §10.1 |

**TTLs and windows.** Balance 15 min, forced on any pay (FSM §3.1); bill dues 24 h; session `expires_at` per DM §3.7 (AWAITING_APPROVAL 5 min, REASONING/EXECUTION 2 min; the cleanup job never aborts an EXECUTION session, DM §7.4); zombie threshold 5 min (FTS §7.1); Healer every 5 min; idempotency key valid 72 h (FTS §5.4); ledger rows ≥ 7 days (MR §6.4); locks stale after 30 min (FTS §9.3). **Rate limits** as in §5. **Reconcile hook (MR v1.2 §6.7).** `FinanceAgent.reconcile(ReconcileRequest{idempotency_key, session_id, family_id, outcome, external_ref, occurred_at})`, called by the Healer after a definite BBPS answer and **before** the kernel's Phase 2. In **one** module transaction, by outcome: `executed` → `transactions` → CONFIRMED with the reference and `confirmed_at = occurred_at`, ledger → `final` with a synthesised success envelope (`side_effects: executed_confirmed`), reservation kept; `failed` → `transactions` → FAILED, ledger → `final` with the failure envelope, reservation released once; `not_executed` (only when the Healer has decided **not** to resubmit, i.e. beyond BBPS's 24-hour window or the payer's account was deleted) → as `failed`. Idempotent: already `final` with the same outcome → ok, nothing changes; already `final` with a different outcome → raises, the Healer leaves the session in EXECUTION and alerts the admins (FIN_012). **Same-key resubmission** (NOT_FOUND inside 24 hours) is not a reconcile: the Healer sends an `execute` dispatch with `recovery: {mode: resubmit}`; the ledger row is `pending`, so the module skips transaction A (no second reservation, no second row), calls BBPS with the same key, and finishes with transaction B as usual. A plain re-dispatch without `recovery` on a `pending` row returns `MOD_EXECUTION_UNCONFIRMED` and calls nothing. The module never polls BBPS by itself and never retries by itself; the Healer never creates or approves a payment. `purge_user` (MR v1.2 §6.7): `billers`, `prepared_payments` and ledger rows of the purged payer are deleted; `transactions` rows are kept 7 years as financial records with `bill_ref` nulled (the payer reference points at an emptied placeholder row, DM v1.4 §7.1).
...
- **OI-3 — Who writes the success audit row, and who fixes `finance.transactions`.** **Closed 2026-09-21:** the kernel writes every `BILL_PAYMENT_*` row, including `BILL_PAYMENT_INITIATED`, in Phase 1 and Phase 2 (FTS v1.3 §4.6); `reconcile()` is an SDK hook (MR v1.2 §6.7) and finalises the ledger too (§8). Original text: FTS §4.3 writes `BILL_PAYMENT_EXECUTED` and the session update in one kernel transaction; MR §6.3/§10 has the module write its own financial audit rows. This PRD follows FTS (module writes only `BILL_PAYMENT_INITIATED`); a `reconcile()` SDK hook is proposed so the Healer can settle module rows without touching the `finance` schema. The code name is settled: `BILL_PAYMENT_EXECUTED` (DM v1.3 §6, register item 7).
...
- **OI-8 — Per-family module settings convention.** **Closed 2026-09-21** (MR v1.2 §6.6, DM v1.4 §9.3). Original text: This PRD adds `finance.family_settings`; see Vault PRD OI-8 (Module Registry round-2 review).
- **OI-9 — Audit codes for settings changes.** **Closed 2026-09-21:** registered with a typed payload (DM v1.4 §6.1). Original text: `FINANCE_SETTINGS_CHANGED` is proposed with OI-3's list for the DM v1.3 taxonomy before the freeze.

### Q&A

| Asked By | Question | Answer |
|---|---|---|
| Engineering | Why does `PAY_BILL` require consent provider `aa` and not `bbps`? | Consent providers are data-access frameworks with handles and revocation (AA, ABHA, DigiLocker, ONDC). BBPS is a payment rail authorised per transaction by the passkey at G4; there is no standing BBPS consent to reverify. MR narrows the enum accordingly, and since v1.2 authorises by the exact purpose (`AA_BALANCE_FETCH`), not by provider; `bbps` stays in `dpi_providers` for gateway routing. |
...
- [x] Independent review by Codex (2026-09-21, changes required); findings 1, 2 and 7 of PR #25 applied in v0.3; OI-3, OI-8, OI-9 closed.
- [ ] Codex's targeted re-review of v0.3; OI-1 before `PAY_RECURRING`; OI-4 in the threat model.
