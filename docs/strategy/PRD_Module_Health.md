...
- **OI-4 — Proxy data in kernel views.** **Closed 2026-09-17:** the kernel view `v_guardians` (DM v1.4 §3.17) carries `proxy_rank`, `conflict_resolution_rule` and `notify_timeout_mins` for managed profiles, next to parents and legal guardians of minors. Add `v_guardians` to this module's `core_read_views` in PRD v0.2.
