...
- **OI-2 — Consent on behalf of a managed profile.** CM v1.2 defines parental consent for minors (`parental_consent_user_id`) but nothing for managed profiles. This PRD assumes the primary proxy grants with their own passkey and the record's `user_id` is the managed profile; CM v1.2 needs an explicit rule (a `proxy_consent_user_id`, or reuse of the parental field with a documented meaning).
...
- **OI-4 — Proxy data in kernel views.** Nudging needs `proxy_rank`, `conflict_resolution_rule` and `notify_timeout_mins`; MR OI-2 has not defined `v_family_members`. Either the view carries them or the Supervisor passes them in the envelope's `acting_as`.
- **OI-5 — MISSED threshold.** 2 × `notify_timeout_mins` is an assumption; the Elder Care Protocol may want a per-medication window. Decide before the metric in §8 is baselined.
