...
| Date | Change |
|---|---|
| 2026-09-17 | `Test_Automation_Strategy.md` drafted at v0.1 (WP-13): tiers and directories, invariant-named tests with a CI checker, determinism rules (injected clock, `run_once()`, `CRASH_AFTER`, stub LLM), scenario catalogue tied to simulator rows and work packages, CI jobs and gates, flaky-test policy, security checks, agent responsibilities, non-goals. The tracker's own test section is marked superseded. |
