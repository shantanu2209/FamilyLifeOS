...
| 2026-09-17 | **A minor's personal documents are visible to the minor's parents and any marked legal guardian** (not to all adults, and not to admins as such; admins only when the minor has neither in the family). A legal guardian is marked by an admin with a passkey and the marking is audited. Emergency ("break-glass") access to another adult's personal documents stays parked for the threat model. Birth certificates are personal documents; each module keeps its own per-family settings table pending the Module Registry review. | Founder ruling. Needs the family graph to tell modules who a dependent's guardians are: kernel view `v_guardians` and relationship types `guardian` / `ward` (Data Model v1.3 change 16). |
---
...
| Date | Change |
|---|---|
| 2026-09-17 | Health PRD v0.2 and Finance PRD v0.2 (#24, stacked on PR #22). Health: proxy consent (CM v1.3 §2.6), `v_guardians` as the source of proxy data, MISSED threshold and slot anchors as bounded family settings, simulator alignment. Finance: settled open issues closed, rulings on the elder balance check and no override, lock release by UPDATE, settings that may only make payment safety stricter. The simulator spec (PR #20) gained the breaker probe endpoints of RB §8.3, bundles selected by HIP id, a second DigiLocker account and a revoked-document response. |
| 2026-09-17 | Founder ruling on minors' documents applied (PR #22): viewers are the parents and any marked legal guardian, admins only as a fallback. Data Model v1.3 change 16: relationship types `guardian` / `ward`, kernel view `v_guardians` (also carries proxy rank, conflict rule and timeout, closing Health PRD OI-4); Module Registry manifest schema whitelists the view. Both specs are still in review, so no version bump. |
| 2026-09-17 | Vault PRD v0.2 (#21): visibility model (document classes; `family_adults` / `holder_only` / `guardians`; holder-controlled sharing; no admin override of another adult's personal documents, break-glass parked as OI-7), reminder defaults with bounded family and per-document settings, §4.6 "Settings and defaults" including what is never a setting, adults link their own documents, `SET_DOCUMENT_VISIBILITY`, VAULT_006. New open issue for MR round 2: a per-family module settings convention (OI-8). |
