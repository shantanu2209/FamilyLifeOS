...
- **In review:** the WP-11 pull request (#30), then PRs #22, #25, the simulator spec v0.2 (issue #31) and the test strategy (#23), in that merge order. (PR #20 merged the simulator spec at v0.1 before Codex's findings were applied; v0.2 carries them.) See the project board for the live list.
...
| 2026-09-17 | **A minor's personal documents are visible to the minor's parents and any marked legal guardian** (not to all adults, and not to admins as such; admins only when the minor has neither in the family). A legal guardian is marked by an admin with a passkey and the marking is audited. Emergency ("break-glass") access to another adult's personal documents stays parked for the threat model. | Founder ruling on Vault PRD OI-6, refined the same day. Vault PRD §4.6; relationship types `guardian` / `ward` and the kernel view `v_guardians` (Data Model v1.4 §3.3, §3.17). |
---
...
| Date | Change |
|---|---|
| 2026-09-21 | `Test_Automation_Strategy.md` v0.2 (WP-13, PR #23): Codex's 8 findings applied. One clock contract across Python, SQL, Redis, simulators and the browser; crash tests kill a real process at named points and restart from PostgreSQL; the crash suite runs on every pull request; the invariant gate reads executed results and an explicit applicability map; simulator ownership instead of a marker; the AA budget test proves local denial; typed audit payloads and adversarial PII inputs; no retry can turn a safety failure green. Scenario additions from the whole review batch. v0.1 was drafted 2026-09-17. |
| 2026-09-21 | `Tech_Spec_Simulator_Architecture.md` v0.2 (#31): Codex's 8 findings on v0.1 applied (v0.1 had been merged through PR #20 before they were). Payment state bound to the idempotency key through fixture-installed per-key scenarios; status contract per outcome with `executed_at`; honest statement of what the revocation tests prove (register item 19) and new row A13; scope-correct prescription bundle plus an over-broad negative; business dates from the test clock; complete generation contract; chaos cleanup and statistical reproducibility; crash B asserts one payment call. OI-1 (payment instrument values versus invariant 9) settled for this build. |
| 2026-09-21 | Health PRD v0.3 and Finance PRD v0.3 (#24, PR #25): Codex's 7 findings applied. Health: MISSED as extra wait after escalation with per-dose stored deadlines; reminders under their own purpose HEALTH_MEDICATION_REMINDERS; admin copies for dependants only, an adult's doses shared only by that adult (founder ruling); proxy replacement versus deletion as in CM v1.4. Finance: atomic per-day reservation for the family limit; stricter-of-two-moments rule for settings and version-checked settings writes; `reconcile()` finalises transaction, ledger and reservation; the kernel writes every BILL_PAYMENT audit row. v0.2 (2026-09-17) introduced the settings sections, proxy consent and simulator alignment. |
| 2026-09-21 | Vault PRD v0.3 (#21, PR #22): Codex's 8 findings applied. Hidden and absent documents are indistinguishable for every class; link idempotency per holder; §4.7 separates actor, holder, account owner and consent subject; effective visibility from live roles with the `on_role_changed` hook; kernel refusals are not module log rows; VAULT codes as `domain_code`; reminder copy policy. OI-2, OI-3, OI-8 closed; OI-10 opened (who consents for elder and passive holders). v0.2 (2026-09-17) introduced the visibility model, reminder defaults and `SET_DOCUMENT_VISIBILITY`. The Data Model and Module Registry edits that were in this PR moved to the WP-11 PR. |
